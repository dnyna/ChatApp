...
const Chats = ({ route }) => {  // here we are recieving routes from navigation
  ...
  const chatId = CurrentUser.uid > recieverId ? `${CurrentUser.uid}-${recieverId}` : `${recieverId}-${CurrentUser.uid}`
  ...
  const TickMarks = (message) => {
    ...
    return message.seen ? (
      <IonIcons name='checkmark-done' color='blue' />
    ) : (<IonIcons name='checkmark' color='grey'/>)
  }
  return (
